...
interface EventCreationWrapperProps {
  template: Template
  userId: string
}

export default function EventCreationWrapper({ template, userId }: EventCreationWrapperProps) {
  // Shared state for live preview
  const [eventTitle, setEventTitle] = useState('')
  const [eventDate, setEventDate] = useState('')
  const [eventLocation, setEventLocation] = useState('')
  const [primaryColor, setPrimaryColor] = useState(template.colors[0] || '#3B82F6')
  const [secondaryColor, setSecondaryColor] = useState(template.colors[1] || '#8B5CF6')
    ...
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
      ...
      <div>
        <div className="space-y-6">
          {/* Pass state setters to form for live updates */}
          <EventCreationFormWithPreview
            template={template}
            userId={userId}
...
// Modified form component that accepts change handlers
function EventCreationFormWithPreview({
  template,
  userId,
  onTitleChange,
  onDateChange,
  ...
  const handlePrimaryColorChange = (value: string) => {
    setCustomization({ ...customization, primaryColor: value })
    onPrimaryColorChange(value)
  }

  const handleSecondaryColorChange = (value: string) => {
    setCustomization({ ...customization, secondaryColor: value })
    onSecondaryColorChange(value)
  }

  const handleCustomMessageChange = (value: string) => {
    setCustomization({ ...customization, customMessage: value })
  ...
  const handleSubmit = async (status: 'DRAFT' | 'ACTIVE') => {
    ...
    try {
      ...
      if (!formData.title || !formData.date) {
        throw new Error('Please fill in all required fields (Title and Date)')
      }
